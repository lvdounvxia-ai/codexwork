...
const assetImages = [
  ...
  ["history", "上次导入角色", "#2f2638", "#a77bff"],
  ["history", "最近生成场景", "#1f2f38", "#58a6d6"],
  ["history", "收藏道具组合", "#332a22", "#c29255"],
...
function assetLibraryDrawer() {
  ...
  const activeAsset = assets.find(asset => asset.id === state.activeLibraryAssetId) || null;
  const variants = activeAsset ? assetLibraryVariants(activeAsset) : [];
  const selected = getSelectedImportAssets();
  return `<div class="asset-import-mask">
        ...
        <div class="asset-library-content">
          ...
          <aside class="asset-library-side">
            ...
            <div class="library-side-preview">
              ...
              ${activeAsset ?
                `<div class="library-variant-grid">
                  ${variants.map(variant => assetLibraryVariantCard(variant)).join("")}
                </div>` :
                `<div class="library-empty-state">${icon.box}<span>先从左侧选择角色</span></div>`
              }
            </div>
          </aside>
        </div>
        <footer class="asset-library-footer">
          <button class="asset-library-action" data-action="import-assets" ${selected.length ? "" : "disabled"}>导入到画布</button>
        </footer>
      </aside>
    </div>`;
}
...
function assetLibraryCard(asset, index) {
  ...
  const count = assetLibraryVariantCount(asset);
  return `<button class="asset-library-card ${state.activeLibraryAssetId === asset.id ? "selected" : ""}" data-action="select-library-asset" data-asset-id="${asset.id}">
      <img src="${asset.src}" alt="${escapeHtml(asset.name)}" />
      <span class="library-card-fade"></span>
      <span class="library-card-count">${count} 个形象</span>
...
function assetLibraryVariantCard(variant) {
  const selected = (state.selectedLibraryVariants || []).includes(variant.id);
  return `<button class="library-variant-card ${selected ? "selected" : ""}" data-action="toggle-library-variant" data-variant-id="${variant.id}">
      <img src="${variant.src}" alt="${escapeHtml(variant.name)}" />
      <span>${escapeHtml(variant.name)}</span>
      <em>${selected ? "✓" : ""}</em>
    </button>`;
}

function assetLibraryVariantCount(asset) {
  const index = assetImages.findIndex(item => item.id === asset.id);
  return index % 2 === 0 ? 3 : 2;
}

function assetLibraryVariants(asset) {
  const index = assetImages.findIndex(item => item.id === asset.id);
  const count = assetLibraryVariantCount(asset);
  const palettes = [
    ["#d9d9dd", "#3b3840"],
  ...
  return Array.from({ length: count }, (_, variantIndex) => {
    const palette = palettes[(index + variantIndex) % palettes.length];
    return {
      id: `${asset.id}-look-${variantIndex + 1}`,
      category: "person",
      name: `${asset.name} 形象${variantIndex + 1}`,
      src: makeMockImage(`${asset.name.slice(0, 6)} 形象${variantIndex + 1}`, palette[0], palette[1]),
  ...
  return `
    <section class="asset-section">
      <div class="asset-section-head">
        <span>${label}</span>
        <div>
          <button class="asset-mini-action ${state.assetMultiSelect ? "active" : ""}" data-action="toggle-asset-multi">多选</button>
...
function categoryLabel(category) {
  return ({ person: "人物", scene: "场景", prop: "道具", history: "历史" })[category] || "人物";
}
...
function canvasGuideMarkup() {
  const steps = [
    ...
    {
      title: "导入项目",
      body: "点击左侧导入项目按钮，从已有项目资产中选择素材，快速带入当前画布继续创作。",
      accent: "项目",
      className: "project",
      targetAction: "open-import",
      targetTitle: "打开导入项目",
    },
    {
      title: "导入资产",
      body: "点击左侧导入资产按钮，进入角色资产库，选择公共资产中的角色形象并导入到画布。",
      accent: "资产",
      className: "asset",
      targetAction: "open-asset-library",
      targetTitle: "打开导入资产",
    },
  ];
  ...
  return `
    <div class="guide-layer guide-${step.className}">
      ...
      ${step.targetAction ? `<button class="guide-target-hit" data-action="guide-target" data-guide-action="${step.targetAction}" title="${escapeHtml(step.targetTitle)}"></button>` : ""}
...
function handleAction(event) {
  ...
  if (action === "guide-target") {
    triggerGuideTarget(el.dataset.guideAction);
    return;
  }
  ...
  if (action === "open-import") {
    ...
    state.addMenu = null;
    state.activeLibraryAssetId = null;
    state.selectedLibraryVariants = [];
    render();
  }
  if (action === "open-asset-library") {
    const openingLibrary = state.drawer !== "asset-library";
    state.drawer = openingLibrary ? "asset-library" : null;
    state.addMenu = null;
  ...
  if (action === "select-library-asset") {
    selectLibraryAsset(el.dataset.assetId);
    return;
  }
  if (action === "toggle-library-variant") {
    toggleLibraryVariant(el.dataset.variantId);
    return;
  }
  if (action === "toggle-asset") toggleAsset(el.dataset.assetId);
  if (action === "toggle-asset-multi") toggleAssetMultiSelect();
...
function triggerGuideTarget(action) {
  state.guideStep = null;
  if (action === "open-import") {
    state.drawer = "import";
    state.addMenu = null;
...
function selectLibraryAsset(assetId) {
  state.activeLibraryAssetId = assetId;
  render();
}

function toggleLibraryVariant(variantId) {
  const selected = new Set(state.selectedLibraryVariants || []);
  if (selected.has(variantId)) {
    selected.delete(variantId);
  } else {
    selected.add(variantId);
  }
  state.selectedLibraryVariants = Array.from(selected);
  render();
}

function toggleAsset(assetId) {
  if (!state.assetMultiSelect) {
    state.selectedAssets = state.selectedAssets.includes(assetId) ? [] : [assetId];
  } else if (state.selectedAssets.includes(assetId)) {
...
function toggleAssetMultiSelect() {
  state.assetMultiSelect = !state.assetMultiSelect;
  if (!state.assetMultiSelect && state.selectedAssets.length > 1) {
    state.selectedAssets = state.selectedAssets.slice(-1);
  }
  render();
}
...
function getSelectedLibraryVariantAssets() {
  const selectedIds = new Set(state.selectedLibraryVariants || []);
  return assetImages
    .filter(asset => asset.category === "person")
    .flatMap(asset => assetLibraryVariants(asset))
    .filter(variant => selectedIds.has(variant.id));
}

function getSelectedImportAssets() {
  const selectedDirectAssets = assetImages.filter(asset => state.selectedAssets.includes(asset.id));
  const selectedLibraryVariants = getSelectedLibraryVariantAssets();
  const seen = new Set();
  return [...selectedDirectAssets, ...selectedLibraryVariants].filter(asset => {
